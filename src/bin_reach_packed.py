...
sized purely from the packet dimensions (n = floor(bin / packet) per axis) -- a complete
pallet, every layer included. EVERY packet is scored, each evaluated as the surface at
its own depth -- i.e. as it would be exposed during top-down unpacking, with the packets
above it already removed (inter-packet collisions aren't simulated; same single-packet
isolation as the other sims). Each packet is tested with a fine off-center tool search so
one flush against a wall can still be picked by sitting the plate inward (the plate is
bigger than a packet). Coverage is the fraction of the WHOLE packed bin the arm can pick
over the course of unpacking it.
...
- For each base pose, samples a 3D grid of PACKETS inside the bin -- each grid point
  is the center of a real rectangular packet (a box) resting in the bin, axis-aligned
  with it, its TOP face at that depth (the suction contact plane). A packet has a valid
  centered PLACEMENT (the plate centered on it) if some IK solution (over several seeds)
  meets ALL of:
      (1) the SUCTION FACE lands on the packet center (FK error < FK_TOL),
  ...
  A packet is PICKABLE (the reported metric) if some reachable, collision-free, level
  placement covers enough of it: at least PACKET_CONTACT_FRAC of the packet's top face
  is under the foam face (the tool bottom) -- not only when the plate is centered on it.
  So an area vacuum gripper picks a packet near a wall by sitting the plate inward, as
  long as >= that fraction of the packet still sits under the foam.
  Pickability = the reachable-center grid DILATED by the set of contact-satisfying
  packet offsets (per clocking, within each depth layer); with no tool it collapses to
  the centered placements.
...
MARGIN = 0.06
# Off-center tool search step (m): a packet is pickable if a level,
...
# ----------------------------------------------------------------------------
# Every grid target is the CENTER of a real packet: an axis-aligned rectangular box
# resting in the bin with its TOP face at the target's z (the suction contact plane).
...
def _packed_layout():
    ...
    (z = each layer's TOP face, the pick plane), the per-axis ticks and the counts.
    z-major order (bottom layer first), like the other sims. EVERY packet is scored."""
    cx, cy = BIN_CENTER_XY
    ...
    return pts, xs, ys, zs, nx, ny, nz

(_PACKED_PTS, _PACKED_XS, _PACKED_YS, _PACKED_ZS, N_X, N_Y, N_Z) = _packed_layout()
...
# Per-packet IK-solution dump (for the viewer's click-to-inspect / failure paths).
# enumerate_solutions() re-solves each packet of the REPORTED poses with this many
# seeds and keeps every DISTINCT joint config (valid + failed, with the failure
# reason) so the viewer can show "every way the arm can reach this TCP" and how a
# failed packet fails. More seeds find more of the (up to ~8) configs; the list is
# capped at MAX_SOLUTIONS. Set DUMP_SOLUTIONS=False to skip it (smaller/faster dump).
DUMP_SOLUTIONS    = True
N_SOLUTION_SEEDS  = 24   # seeds used only by enumerate_solutions (dump phase only)
MAX_SOLUTIONS     = 16   # cap on distinct configs stored per packet
...
# target is a PACKET CENTER (z = its layer's top face). Every packet is scored, each as
# the exposed surface at its own depth (top-down unpacking).
# ----------------------------------------------------------------------------
def bin_targets():
...
def solve_pick(rid, bin_id, gripper_id, ee_link, joints, lo, hi, target,
    ...
    here with the foam-face center exactly on the packet center (the target)?" Returns
    the first valid joint configuration (and its diagnostics, incl. `ori_idx` = which
    clocking won) as a dict, or None. 'Valid' = the suction face center lands on the
    packet center with the tool pointing down, in joint limits, and collision-free
    against the bin, the arm itself, and the gripper body. Each seed is a different
    starting guess, so we explore the arm's multiple IK solutions instead of trusting
    the single config one IK call happens to return. Checks run cheapest-first and bail
    on the first failure.

    A packet is *pickable* (the reported metric) if some such collision-free placement
    -- not only the one centered on it -- covers >= PACKET_CONTACT_FRAC of the packet's
    top face with the foam; see `_covered_mask`. This function finds the centered
    placements; pickability dilates them by the contact offsets.
...
def enumerate_solutions(rid, bin_id, gripper_id, ee_link, joints, lo, hi, target,
    ...
    `target` pointing down -- for the viewer's "click a packet, see how the arm
    reaches (or fails to reach) it" inspector. Unlike solve_pick (which bails on the
    first valid config), this tries all clockings x all seeds, keeps each distinct
    joint config once (rounded), and annotates WHY it fails so failures are visible:
        fail = None            -> a valid placement (ok)
               "reach"         -> the foam goal is beyond REACH_MAX (arm shown stretched)
               "unreachable"   -> IK didn't land the foam on the target (fk > FK_TOL)
               "tilt"          -> tool can't be presented level here
               "limits"        -> a joint is past its limit
               "collision_bin" / "collision_self" / "collision_gripper"
    Returns a list of dicts {joints, fk_err, tilt_deg, ok, fail, ori_idx, accepted}
    (`accepted` marks the one solve_pick would have returned), capped at
    MAX_SOLUTIONS with ok solutions kept first. A 6-DOF arm reaching a fixed TCP has
    several configs (elbow up/down, shoulder front/back, wrist flip); more seeds
    (N_SOLUTION_SEEDS) discover more of them."""
    target = np.asarray(target, dtype=float)
    standoff = GRIPPER_STANDOFF if USE_GRIPPER else 0.0
    flange_goal = target + np.array([0.0, 0.0, standoff])
    base_pos = np.array(p.getBasePositionAndOrientation(rid)[0])
    ...
    for ori_idx, ori in enumerate(orientations):
        for seed in seeds:
            ...
            for j, q in zip(joints, sol):
                p.resetJointState(rid, j, q)
            key = tuple(round(q, 2) for q in sol)        # distinct configs only
            if key in seen:
                continue
            seen.add(key)
            ls = p.getLinkState(rid, ee_link, computeForwardKinematics=True)
    ...
    # ok (and the accepted) first, then by fk error; cap the list size.
...
# ----------------------------------------------------------------------------
# Pickability: a PACKET is pickable if some reachable, collision-free, level
# placement covers >= PACKET_CONTACT_FRAC of the PACKET's top face with the TOOL
# BOTTOM (foam face) -- not only when the plate is centered on it. We get this by
# DILATING the reachable-center grid by the set of contact-satisfying packet
# offsets (per clocking, within each depth layer): for a tool centered at grid cell
# C, every packet whose center sits at an offset that keeps >= the required fraction
# of the packet under the foam is pickable from C. So near-wall packets the gripper
# grips off-center count too, but only while enough of the packet stays under the foam.
# ----------------------------------------------------------------------------
def _rect_poly(cx, cy, ang, L, W):
    """CCW corner list of an L x W rectangle centered at (cx, cy) with its L-axis at
    heading `ang` (radians) in the world XY plane."""
    c, s = math.cos(ang), math.sin(ang)
    hl, hw = L / 2.0, W / 2.0
    local = [(hl, -hw), (hl, hw), (-hl, hw), (-hl, -hw)]   # CCW
    return [(cx + c * x - s * y, cy + s * x + c * y) for x, y in local]

def _convex_intersect_area(subject, clip):
    """Area of the intersection of two convex polygons given CCW, via Sutherland-
    Hodgman clipping + the shoelace formula. Returns 0.0 if they don't overlap.
    Exact for the rotated tool footprint vs the axis-aligned packet at any clocking."""
    def inside(pt, a, b):                       # left of directed edge a->b => inside (CCW)
        return (b[0] - a[0]) * (pt[1] - a[1]) - (b[1] - a[1]) * (pt[0] - a[0]) >= -1e-12
    def inter(s, e, a, b):                      # intersection of segment s-e with line a-b
    ...
    for i in range(len(out)):
        x1, y1 = out[i]; x2, y2 = out[(i + 1) % len(out)]
        area += x1 * y2 - x2 * y1
    return abs(area) * 0.5

# ----------------------------------------------------------------------------
# Pickability (packed): because packets sit at packet pitch (too coarse for the
# grid-dilation the sampling sims use), each packet is tested directly with
...
def _pick_packet(w, t):
    """Can the gripper pick the packet centred at `t`? Returns
...
def _eval_pose(args):
    """Evaluate one base pose -> (pose_idx, pickable_mask, center_mask). EVERY packet in
    the pack is scored, each as the exposed surface at its own depth: pickable = a level,
    collision-free placement covers >= PACKET_CONTACT_FRAC of it (centered or via the fine
    off-center search); center_mask = the subset where the plate can be CENTERED on the
    packet (a real placement, drives the animation/dump)."""
...
def _aggregate(targets, meta, results):
    ...
    for idx in range(len(meta)):
        ...
        cov = float(covered.mean())                   # over the WHOLE packed bin
...
def _sweep_gui(targets, poses, meta, total):
    ...
    for ps in poses:
        ...
        hud_update(hud, [
            ...
            f"({int(covered.sum())}/{len(targets)} pickable)",
...
def run():
    ...
    def _print_pose(tag, b):
        print(f"  {tag}: x={b['xy'][0]:+.3f} y={b['xy'][1]:+.3f} z={b['z']:.3f} m  "
              ...
              f"({int(b['mask'].sum())}/{len(targets)} packets)")
...
def _layer_grid(n):
    """A roughly-square (nrows, ncols) layout for n depth-layer panels (the packed bin
    can have many layers, so a 1xN montage would be unusably wide)."""
    ncols = min(n, 6)
    nrows = int(math.ceil(n / ncols))
    return nrows, ncols

def plot_best_slices(best, xs, ys, zs):
    """Pickable packets at the best base, every packed layer in a grid (bottom-left =
    deepest layer, increasing depth-index across/up). Each packet is scored as the
    surface at its depth."""
    mask = best["mask"].reshape(N_Z, N_Y, N_X)
    extent = _cell_extent(xs, ys)
    nrows, ncols = _layer_grid(N_Z)
    ...
    for k in range(nrows * ncols):
        ...
        _bin_footprint(a, ec="black", lw=0.8, alpha=0.5)
        a.set_xticks([]); a.set_yticks([])
        a.set_title(f"z={zs[k]:.2f}m  {int(mask[k].sum())}/{mask[k].size}", fontsize=8)
    fig.suptitle(f"Pickable packets by layer at best base  "
                 f"(x={best['xy'][0]:+.2f}, y={best['xy'][1]:+.2f}, "
                 f"z={best['z']:.2f} m, yaw={math.degrees(best['yaw']):+.0f}deg)   "
                 f"green = pickable ({PACKET_CONTACT_FRAC*100:.0f}% under foam), red = not",
                 fontsize=10)
    fig.tight_layout()
...
def plot_reach_3d(best, targets):
    """3D scatter of pickable vs not-pickable packets at the best base,
...
def plot_target_frequency(target_freq, xs, ys, zs):
    """For every packet, the % of all swept base positions that can pick it -- every
    packed layer in a grid; highlights intrinsically hard spots at each depth."""
    ...
    for k in range(nrows * ncols):
        ...
        im = a.imshow(freq[k], origin="lower", cmap="magma", vmin=0, vmax=100,
                      extent=extent, aspect="equal")
        _bin_footprint(a, ec="cyan", lw=0.8, alpha=0.6)
        a.set_xticks([]); a.set_yticks([])
        a.set_title(f"z={zs[k]:.2f}m  (mean {freq[k].mean():.0f}%)", fontsize=8)
    fig.suptitle("Packet pickability across ALL base positions, by layer "
...
def _pick_records(world, base, targets, covered, sol_seeds=None):
    """Per-packet detail for one base pose (packed pallet). EVERY packet is scored (as the
    surface at its depth): `covered` is the pickability, the real pick (centered OR the
    fine off-center placement) supplies the top-level joint config, and -- with
    `sol_seeds` -- every distinct IK config for the centered TCP is enumerated into
    `solutions` (valid + failed, with the reason) for the viewer's click-inspect / failure
    paths. An off-center pick is prepended as the accepted solution so the viewer can show
    the actual grasp, not just the (failing) centered attempts."""
...
def _best_block(world, b, rank, targets, zs, sol_seeds=None):
    """One base pose's full record (summary + per-depth PICKABLE counts + every packet's
    pickable/placement flags and joint solution) plus its (n_targets x n_joints) joint
    ...
    block = {
        ...
        "pickable": int(b["mask"].sum()), "total": int(b["mask"].size),  # whole pack
...
def dump_best_data(world, bests, diverse, targets, coverage, target_freq):
    ...
    config = {
        ...
        "gripper": gripper_cfg,
        "packet": {"length_m": PACKET_L, "width_m": PACKET_W, "height_m": PACKET_H,
                   "length_in": round(PACKET_L / _IN, 3),
                   "width_in": round(PACKET_W / _IN, 3),
                   "height_in": round(PACKET_H / _IN, 3),
        ...
        "packed": {"nx": N_X, "ny": N_Y, "nz": N_Z, "n_packets": int(len(_PACKED_PTS)),
                   "note": "bin filled edge-to-edge from the packet size; every packet "
                           "scored as the exposed surface at its depth (top-down unpacking)"},
    ...
    # Extra seeds to enumerate every distinct IK config per packet (dump only).
    ...
    for r, b in enumerate(bests, 1):
        ...
        npz[f"best{r}_pickable_mask"] = b["mask"]
    ...
    for r, b in enumerate(diverse, 1):
        ...
        npz[f"diverse{r}_pickable_mask"] = b["mask"]
...
def _add_target_markers(targets, mask):
    """Visual-only PACKET boxes for the fully-packed pallet (every layer): green =
    pickable, red = not. Each box's TOP face is at the target z (debug points/lines
    aren't captured by getCameraImage)."""
...
def render_animation(world, best, targets):
    ...
    # markers: green = pickable packets (the reported metric); the arm visits the real
    # collision-free centered placements (center_mask), which pick that whole region.
